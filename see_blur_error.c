	...
	for ( int new_element = 0 ; new_element < x_size*y_size ; new_element++ ) 
{
   		...
   		for (int rows = -half_kernel ; rows <= half_kernel ; rows++) {  
      			...
      			if (row_middle<0)
         			row_middle += (half_kernel+1)*x_size;
...
void check_and_convert ( unsigned short int * image , const float * blurred , const short int x_size , const short int y_size)
{
        float threshold = 65535.0;
        
        for (int i = 0 ; i < x_size*y_size ; i++)
                image[i] = (blurred[i] <= threshold) ? blurred[i] : threshold;
...
for (int i = 0 ; i < 4 ; i++ ) {
        ...
        for (int j = print1 + xsize*i ; j < print2 + 1 + xsize*i ; j++) {
...
/* check if values are within the limit (65535) , set those which exceed the limit to 65535 and convert all values from float to integers */




/* show some elements of the initial pointer and of the modified one , and the kernel */
